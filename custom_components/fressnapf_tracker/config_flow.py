...
class FressnapfTrackerFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):  # type: ignore
    ...
    async def async_step_user(self, user_input=None):
        ...
        for entry in entries:
            if (
                entry.data[CONF_SERIALNUMBER] == user_input[CONF_SERIALNUMBER]
            ):
    ...
    async def _show_config_form(self, user_input) -> FlowResult:
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema(
                {vol.Required(CONF_SERIALNUMBER): str, vol.Required(CONF_DEVICE_TOKEN): str, vol.Required(CONF_AUTH_TOKEN): str}
            ),
            errors=self._errors,
        )
